...
export default function JournalEmotionPage() {
    ...
    const load = async () => {
        ...
        try {
            const [rows, t] = await Promise.all([
                getJournalHistory("emotions", 30),
                getTodayEntryStatus("emotions"),
            ]);
            setHistory(rows);
            setToday(t);
        } finally {
    ...
    const start = async () => {
        try {
            setStarting(true);
            const entryId = await startJournalEntry("emotions");
            navigate(`/journal/session/emotions?entry=${entryId}`);
        } catch (e: any) {
        ...
        <section className="mx-auto max-w-5xl px-3 text-text">
            ...
            <div className="mb-4 overflow-hidden rounded-xl border border-border bg-gradient-to-r from-sky-50 to-emerald-50 p-4 sm:flex sm:items-center sm:justify-between">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <div className="text-base font-semibold">Diario emocional</div>
                        {showCTA ? (
                            <p className="text-sm text-text/70">
                                Tienes una sesión de diario nueva para hoy. Registra cómo te sientes.
                            </p>
                        ) : (
                            <p className="text-sm text-text/70">Ya completaste tu diario de hoy. ¡Vuelve mañana! ✨</p>
                        )}
                    </div>
                </div>
                {showCTA && (
